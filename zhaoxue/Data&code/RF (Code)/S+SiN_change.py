...
for i in range(1, 101):
    ...
    for train, test in StratifiedKFold(n_splits=10, shuffle=True, random_state=i).split(X, Y):
        ...
        pd.DataFrame([*zip(Xtrain_column[sfm_select_wrapper.get_support()]
                    , RFC__.feature_importances_)]).to_csv('E:\\zhaoxue(last-first)_2\\activity_plus\\RF\\contributing_weight(S+SiN_change).csv'
                                                           ,mode='a')
...
pd.DataFrame(acc_list).to_csv('E:\\zhaoxue(last-first)_2\\activity_plus\\RF\\acc(S+SiN_change).csv')
pd.DataFrame(pre_list).to_csv('E:\\zhaoxue(last-first)_2\\activity_plus\\RF\\pre(S+SiN_change).csv')
pd.DataFrame(rec_list).to_csv('E:\\zhaoxue(last-first)_2\\activity_plus\\RF\\rec(S+SiN_change).csv')
pd.DataFrame(auc_list).to_csv('E:\\zhaoxue(last-first)_2\\activity_plus\\RF\\auc(S+SiN_change).csv')