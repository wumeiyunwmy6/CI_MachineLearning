...
X_new = pd.concat([
    ...
    # babblespeech
    , data_input.iloc[:, data_input.columns == "bs_ch1_change"]
    , data_input.iloc[:, data_input.columns == "bs_ch2_change"]
    , data_input.iloc[:, data_input.columns == "bs_ch3_change"]
    , data_input.iloc[:, data_input.columns == "bs_ch4_change"]
    , data_input.iloc[:, data_input.columns == "bs_ch5_change"]
    , data_input.iloc[:, data_input.columns == "bs_ch6_change"]
    , data_input.iloc[:, data_input.columns == "bs_ch7_change"]
    , data_input.iloc[:, data_input.columns == "bs_ch8_change"]
    , data_input.iloc[:, data_input.columns == "bs_ch9_change"]
    , data_input.iloc[:, data_input.columns == "bs_ch10_change"]
...
for i in range(1, 101):  # 100次嵌套交叉验证
    ...
    for o_train, o_test in StratifiedKFold(n_splits=10, shuffle=True, random_state=i).split(X, Y):
        ...
        compare_score = 0
        compare_index = 0
        for s in range(1, 41, 1):  # 在训练集X[O_train]上做交叉验证，选出最优特征子集
...
pd.DataFrame(accuracy_list).to_csv('E:\\zhaoxue(last-first)_2\\activity_plus\\classification\\l_S+N+SiN+M_change(acc100).csv')
pd.DataFrame(precision_list).to_csv('E:\\zhaoxue(last-first)_2\\activity_plus\\classification\\l_S+N+SiN+M_change(pre100).csv')
pd.DataFrame(recall_list).to_csv('E:\\zhaoxue(last-first)_2\\activity_plus\\classification\\l_S+N+SiN+M_change(rec100).csv')
pd.DataFrame(auc_list).to_csv('E:\\zhaoxue(last-first)_2\\activity_plus\\classification\\l_S+N+SiN+M_change(auc100).csv')